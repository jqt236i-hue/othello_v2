...
- 盤上の石1つを選択
- 上下左右のいずれかへランダムに端まで移動

### 10.8 INHERIT_WILL（意志の継承）
...
### 10.13 DESTROY_ONE_STONE（破壊神）

- 任意の石1つを破壊

### 10.14 TIME_BOMB（時限爆弾）
...
### 10.15 ULTIMATE_REVERSE_DRAGON（究極反転龍）
...
### 10.16 BREEDING_WILL（繁殖の意志）

- 次に置く石を繁殖石化（アンカー）
- 配置時即時 + 所有者ターン開始時に周囲空き1マスへ生成
- 生成で挟める場合は反転
- 持続3ターン
- 繁殖石は反転/交換不可、破壊は可能

### 10.17 CROSS_BOMB（十字爆弾）

- 次に置く石を十字爆弾化
- 通常反転後に中心+縦横1マスを破壊

### 10.18 HYPERACTIVE_WILL（多動の意志）
...
### 10.19 SELL_CARD_WILL（売却の意志）

- 使用後に手札1枚を売却
- 売却カードのコスト分、布石獲得

### 10.20 PLUNDER_WILL（吸収の意志）

- この手での反転枚数ぶん、相手布石を奪って自分へ加算

### 10.21 WORK_WILL（出稼ぎの意志）

- 次配置石をアンカー化
- 所有者ターン開始時に `1→2→4→8→16` の順で加算
- アンカー喪失（破壊・相手化）で終了

### 10.22 DOUBLE_PLACE（二連投石）

- コスト24
- そのターンのみ配置回数 +1（合計2回）

### 10.23 HEAVEN_BLESSING（天の恵み）

- ランダム候補5枚から1枚選んで手札に加える
- 未選択候補は消滅（捨て札に送らない）
- 手札上限で受け取れない場合は選択不可

### 10.24 CONDEMN_WILL（断罪の意志）

- 相手手札を公開し、1枚選んで破壊

### 10.25 GOLD_STONE（金の意志）

- 次配置の反転による布石獲得を4倍
- 配置石は配置直後に自壊（フェードアウト）

### 10.26 SILVER_STONE（銀の意志）

- 次配置の反転による布石獲得を3倍
- 配置石は配置直後に自壊（フェードアウト）

### 10.27 STEAL_CARD（略奪の意志）

- この手の反転枚数ぶん相手手札を奪う
- 奪ったカードは手札上限まで自分手札へ
- 超過分は破棄せず自分デッキへ

### 10.28 GUARD_WILL（守る意志）

- 自分石1つに完全保護を付与
- 持続3ターン（所有者ターン開始で減算）

### 10.29 ULTIMATE_DESTROY_GOD（究極破壊神）
...
### 10.30 ULTIMATE_HYPERACTIVE_GOD（究極多動神）

- 次に置く石を究極多動神化（アンカー）
- 両者ターン開始時に発動（所有者ターン限定ではない）
- 1ターンで「1マス移動」を2回連続実行する（2マス直線固定移動はしない）
- 各移動ステップ開始時、周囲8マスに空きが無ければ即消滅
- 各移動ステップの着地後、上下左右隣接の敵石を全て対象に吹き飛ばしを試行
- 吹き飛ばしは対象石ごとに独立抽選し、進める方向（上下左右）からランダム1方向を選ぶ
- 吹き飛ばし距離は最大2マス（0マス=失敗、1〜2マス=成功）
- 吹き飛ばし成功1回につき布石+2（上限30）
- 反転・破壊・誘惑は可能、交換は無効
- 反転された時点で究極多動神属性は解除
