...
デッキは初期生成後にシャッフルされ、初期手札/ドローで引くカード順は毎試合ランダムになる（ただし乱数seedが同一なら再現可能）。


デッキは常に **20種類×各1枚（合計20枚）** とする（=同じカード種の重複は入れない）。
...
各プレイヤーは 自分の手番が 2 回訪れるごとに 1 枚ドローする

例：1回目＝ドローなし、2回目＝1枚、3回目＝なし、4回目＝1枚…
...
山札が無くなったら、捨て札をシャッフルして山札に戻す

山札も捨て札も無い場合：ドロー失敗（手札は変化しない）
...
- cost 15
  - `regen_01` — **再生の意志** (`REGEN_WILL`) — 枚数: 1
