...
- 仕様変更だけでなく、**実装変更で仕様の見え方/挙動が変わる場合**も `01-rulebook.md` を更新する。
- 実装を変えたのに `01-rulebook.md` に差分が不要な場合は、最終報告で「不要理由」を明記する。
- 最終報告には、`01-rulebook.md` の更新有無と更新箇所（見出し名または行）を必ず含める。
- 上記を満たさない場合、その作業は完了扱いにしない。

## 原因調査と修正方針（追加ルール）

- 対症療法より根本原因を優先する。症状を一時的に隠す修正は最後の手段にする。
- 修正前に再現条件を固定し、最小の観測（ログ/テスト）で原因を確認してから実装する。
- UI/演出の既存仕様は原則維持する。経路や見た目の一貫性を崩す変更は先に理由を残す。
- `owner` / `player` など境界値は入口で正規化する（`black`/`white` と `1`/`-1` を混在させない）。
- 暫定回避を入れる場合は、暫定であること・解除条件・追跡先をコメントまたは共有メモに必ず残す。
